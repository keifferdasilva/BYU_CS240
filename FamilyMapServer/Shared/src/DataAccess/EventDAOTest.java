...
public class EventDAOTest {
    ...
    @Test
    public void findEventsSuccess() throws DataAccessException{
        eDao.insert(bestEvent);
        Event secondEvent = new Event("1234", "Gale", "Gale123A", 12.1f, 41.3f,
                "Japan", "Osaka", "Test", 1984);
        eDao.insert(secondEvent);
        ArrayList<Event> events = eDao.getEvents("Gale");
        assertTrue(events.size() == 2);
        events.forEach(event -> {System.out.println(event.getEventID());});
    }
    ...
    public void clearEvent() throws DataAccessException{
        ...
        eDao.insert(bestEvent);
        Event secondEvent = new Event("1234", "Other", "Gale123A", 12.1f, 41.3f,
                "Japan", "Osaka", "Test", 1984);
        eDao.insert(secondEvent);
        eDao.clear(bestEvent.getAssociatedUsername());
        assertNull(eDao.find(bestEvent.getEventID()));
        assertEquals(secondEvent, eDao.find(secondEvent.getEventID()));
    }
}
